...
function filteredFunction(arr) {
    ...
    if (arr.length > 0) {
        for (i = 0; i < arr.length; i++) {
            ...
            <p class="label">Label: ${arr[i].labels} </p>
            <p class="author">Author: ${arr[i].author}</p>
  ...
  for (let i = 0; i < checkBoxes.length; i++) {
    if (checkBoxes[i].checked) {
      ...
      for (let j = 0; j < dataParse.issues.length; j++) {
        if (dataParse.issues[j].labels[0] === checkBoxes[i].value) {
...
filterBtnByAuthor.addEventListener("click", function () {
  ...
  const author = dataParse.issues.filter(
    (issue) => issue.author === optionValue.value
  );
